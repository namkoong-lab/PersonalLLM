...
class MetaLearnTenKAlgorithm(BaseAlgorithm):
    """
    ...
    def __init__(self, args):
        ...
        if args.eval_type == "reward":
            self.meta_learning_database = load_dataset("andrewsiah/MetaLearningRewardDatabase10K")["train"]
            self.eval_dataset = load_dataset("andrewsiah/Eval_Reward_Dataset")["test"]
        if args.eval_type == "pairwise_pref":
            self.meta_learning_database = load_dataset("andrewsiah/MetaLearningPrefDatabase10K")["train"]
    ...
    def load_dataset(self, args):
        ...
        self.eval_df = pd.DataFrame.from_dict(self.eval_dataset)
        if args.eval_type == "pairwise_pref":
            self.train_df_embeddings = pd.read_json('/shared/share_mala/leon/train_pref_10k_embeddings.json')
            self.eval_df_embeddings = pd.read_json('/shared/share_mala/leon/eval_pref_embeddings.json')
        if args.eval_type == "reward":
            self.train_df_embeddings = pd.read_json('/shared/share_mala/leon/train_10k_embeddings.json')
            self.eval_df_embeddings = pd.read_json('/shared/share_mala/leon/eval_embeddings.json')
...
if __name__ == "__main__":
    test_gen = MetaLearnTenKAlgorithm()
